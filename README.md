...
- **Filters** use a grouped human builder over `query.canonical.filter.targets`. Ordinary conditions render inline as separate **Field**, **Operator**, and string/expression **Value** controls. Field choices come from Discovery; Operator choices reload from Discovery for the selected field using n8n's row-relative dynamic-option dependency. The adapter parses number/integer/boolean values and canonical JSON operands back into the typed Canonical Filter AST.
