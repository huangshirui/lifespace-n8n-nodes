...
- **Fields**, **Temporal Ranges**, and **Action Input** remain generated from Runtime Discovery. Scalar/relation CRUD fields stay in Resource Mapper; writable `temporal_range` CRUD fields use the dedicated form. **Filters** use Discovery-backed field/operator selectors plus string/expression values so Boolean grouping does not depend on dynamically changing n8n value-control types.

For List / Query, **Filters** is a Discovery-driven group builder. There are no separate top-level Match or Conditions surfaces: every condition belongs to a Filter Group, each group chooses All/Any, and groups are ANDed together. Conditions use the same vertical layout as n8n's Data Table filters: **Field**, then **Operator**, then **Value**. Changing Field reloads only the operators published for that field. Scalar values remain text or n8n expressions and are deterministically converted back to canonical typed values before the request.

Filter Value is a string-oriented Human UI boundary, not a string-only runtime contract. Expression results are normalized from their n8n runtime type before the Canonical Query request: booleans stay boolean; numbers/integer values accept native numbers or numeric strings; date filters accept date strings plus JS Date / Luxon-like DateTime values; instant/datetime filters accept absolute RFC3339 strings plus JS Date / Luxon-like DateTime values; legacy multi-value `in` accepts arrays or comma-separated strings. Relation `contains` remains a single-ID predicate and accepts either an ID string or an object carrying an `id` string; arrays are rejected so the adapter does not silently invent contains-any semantics.

For TemporalRange fields, the human projection does not ask for JSON. Canonical `overlaps` is split into **Overlaps Start** and **Overlaps End** inputs; they must appear once each in the same Filter Group and are lowered back to one canonical `overlaps` predicate. Human date End values are inclusive, so the adapter converts them to Core's exclusive end boundary. `before` and `after` accept a single date or absolute RFC3339 date-time string. `contains` remains executable for stored workflows but is not offered in the new Human UI until it has an equally simple structured projection. **Options → Viewing Timezone** is optional; TemporalRange date interpretation and sorting fall back to the effective n8n workflow timezone when it is omitted. **Sorts** remains an ordered structural list because sort priority is part of workflow structure rather than per-item data.
